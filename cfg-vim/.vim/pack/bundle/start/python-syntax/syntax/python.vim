...
if s:Python2Syntax()
    ...
    " by me:
    syn match pythonParam '\%([^[:cntrl:][:space:][:punct:][:digit:]]\|_\)\%([^[:cntrl:][:punct:][:space:]]\|_\)*\ze\%(\s*:\s*\(str\|dict\|list\|tuple\|int\|bool\|Any\|Callable\|Type\)[,)=\[ ]\)'
    " by me:
    syn match pythonVar   '\%([^[:cntrl:][:space:][:punct:][:digit:]]\|_\)\%([^[:cntrl:][:punct:][:space:]]\|_\)*\ze\%(=\)'
...
" ^^ orig: syn keyword pythonOperator and in is not or

if s:Enabled('g:python_highlight_operators')
    syn match pythonOperator '\V=\|-\|+\|*\|@\|/\|%\|&\||\|^\|~\|<\|>\|!=\|:'
    " ^^ orig: syn match pythonOperator '\V=\|-\|+\|*\|@\|/\|%\|&\||\|^\|~\|<\|>\|!='
endif
...
syn keyword pythonTodo          TODO TEMPORARY FIXME NOTE contained
" ^^ ORIGINAL: TODO FIXME XXX contained
" by me:
syn match pythonTodo 'JUMP_[0-9]\+'
...
if s:Enabled('g:python_highlight_builtin_types')
    syn match pythonBuiltinFunc '\v\.@<!<%(object|bool|int|float|tuple|str|list|dict|set|frozenset|bytearray|bytes|Any|Callable|Dict|List|Tuple|Type)>'
    " orig syn match pythonBuiltinType '\v\.@<!<%(object|bool|int|float|tuple|str|list|dict|set|frozenset|bytearray|bytes)>'
endif
...
if v:version >= 508 || !exists('did_python_syn_inits')
    ...
    " added by me:
    HiLink pythonParam            PythonParams
    " added by me:
    HiLink pythonVar              PythonVars
    delcommand HiLink
endif
